...
try {
    ...
    [System.IO.File]::WriteAllBytes((Join-Path $PSScriptRoot '../desktop/assets/waid.png'), $images[-1])
    ...
    try {
        ...
        [System.IO.File]::WriteAllBytes((Join-Path $PSScriptRoot 'waid.ico'), $output.ToArray())
    } finally {
        $writer.Dispose()
        $output.Dispose()
    }
