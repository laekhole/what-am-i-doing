...
impl App {
        ...
        if let Some(rx) = &self.activation {
            match rx.try_recv() {
                ...
                Err(mpsc::TryRecvError::Disconnected) => {
                    self.notice = tr("세션 복귀가 예기치 않게 중단되었습니다. 다시 시도하세요.", "Session return stopped unexpectedly. Try again.").into();
                    self.activation = None;
                }
    ...
    fn save(&mut self) {
        if let Err(e) = self.settings.save(&self.path) {
            self.notice = waid::trf!("설정을 저장하지 못했습니다: {e}. 폴더 권한을 확인하고 다시 시도하세요.", "Settings were not saved: {e}. Check folder permissions and retry.");
        }
    }

    fn action(&mut self, action: &Value) -> Result<(), String> {
        let name = action["action"].as_str().ok_or(tr("동작이 없습니다", "Missing action"))?;
        if name == "poll" { return Ok(()); }
        let value = action["value"].as_str().unwrap_or_default();
        let before = self.settings.clone();
        match name {
            "language" => self.settings.language = Language::from_code(value).ok_or(tr("지원하지 않는 언어입니다", "Unsupported language"))?,
            "search" => self.settings.search = value.chars().take(200).collect(),
            "state" => {
                if !value.is_empty() && !ui::STATES.contains(&value) { return Err(tr("올바르지 않은 상태입니다", "Invalid status").into()); }
            ...
            "opacity" => self.settings.opacity = value.parse::<u8>().ok().filter(|v| (40..=100).contains(v)).ok_or(tr("불투명도는 40–100이어야 합니다", "Opacity must be 40–100"))?,
            "preview" | "template" => {
                Skin::parse(value)?;
            ...
            "pin" | "hide" | "dismiss" | "connect" | "disconnect" | "open" => {
                let id = action["id"].as_str().ok_or(tr("세션을 선택하세요", "Select a session"))?;
                let row = self.settings.visible(&self.rows).into_iter().find(|r| r.id == id)
                    .ok_or(tr("이 세션은 더 이상 목록에 없습니다. 다시 선택하세요.", "This session is no longer in the list. Select it again."))?;
                match name {
                    "pin" | "hide" => {
                    ...
                    "connect" => {
                        ...
                        if self.settings.session_targets.len() >= 1024 && !self.settings.session_targets.contains_key(id) {
                            return Err(tr("최대 1,024개 연결을 저장할 수 있습니다.", "At most 1,024 connections can be saved.").into());
                        }
                    ...
                    "open" => {
                        if self.activation.is_some() { return Err(tr("이미 세션 복귀가 진행 중입니다.", "Session return is already in progress.").into()); }
                        let target = self.settings.session_targets.get(id).cloned();
                        let (tx, rx) = mpsc::channel();
                        self.activation = Some(rx);
                        self.notice = tr("기존 세션을 확인하는 중…", "Checking the existing session…").into();
                        std::thread::spawn(move || {
                            let result = match target {
                                Some(target) => activate::open_associated(&row, &target),
                                None => activate::open(&row).map(|_| tr("기존 Orca 세션을 선택했습니다.", "Selected the existing Orca session.").into()),
                            };
            ...
            _ => return Err(tr("알 수 없는 동작입니다", "Unknown action").into()),
        }
...
pub fn run() {
    ...
    if let Err(error) = result {
        let message = CString::new(waid::trf!("waid를 시작하지 못했습니다: {}", "waid could not start: {}", error.replace('\0', ""))).unwrap();
...
mod tests {
    ...
    fn native_actions_persist_filter_restore_and_validate_templates_and_links() {
        // Language is process-wide; isolate switches from the parallel test suite.
        if std::env::var("WAID_MAC_ACTIONS_TEST").as_deref() != Ok("1") {
            let result = std::process::Command::new(std::env::current_exe().unwrap())
                .args(["--exact", "macos::tests::native_actions_persist_filter_restore_and_validate_templates_and_links", "--nocapture"])
                .env("WAID_MAC_ACTIONS_TEST", "1").output().unwrap();
        ...
        assert_eq!(Settings::read(&app.path).unwrap().language, Language::English);
        assert!(app.action(&json!({"action":"language","value":"fr"})).is_err());
        assert_eq!(app.settings.language, Language::English);
        app.action(&json!({"action":"language","value":"ko"})).unwrap();
        assert_eq!(app.view()["language"], "ko");
        assert_eq!(app.view()["rows"][0]["prompt"], "find me");
        assert_eq!(Settings::read(&app.path).unwrap().language, Language::Korean);
