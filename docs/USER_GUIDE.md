...
This is the full reference for waid. For a short overview, screenshots and downloads, start at the [README](../README.md). Korean version: [USER_GUIDE.ko.md](USER_GUIDE.ko.md).
...
Published CI results (`47f4317`, 2026-09-09): [Windows CI](https://github.com/laekhole/what-am-i-doing/actions/runs/34308905362) passed **130 checks**; [Mac CI](https://github.com/laekhole/what-am-i-doing/actions/runs/34308905356) passed **121 Rust checks per architecture** plus AppKit smoke/restart checks on Apple Silicon and Intel. Environment-dependent tests remain ignored: three on Windows and one on each Mac architecture. See [VALIDATION.md](../VALIDATION.md) for later source checks and remaining work; these published results do not complete the real-device walkthroughs in that record and [MACOS.md](../MACOS.md).
...
- The collector reads existing local logs and opens supported SQLite sources read-only; it does not modify source conversations or send prompts to agents. Orca SSH support reads Orca's existing local hook mirror, without connecting to the remote machine. On Windows, while an SSH client is running, a bounded UI Automation scan also observes visible terminal screens locally; it never types, changes tabs, scrolls, or opens an SSH connection.
...
`waid-desktop.exe` runs on its own. It starts a second instance of itself in internal collector mode and only terminates that child when exiting. No companion executable is extracted or required. The independent `waid.exe` CLI remains available for terminal use; building the desktop also compiles the shared core directly. On Windows, **Licenses and notices (라이선스 및 고지)** in the tray menu opens `LICENSES.txt` in the app data folder. It contains waid's MIT license, dependency and Rust standard-library notices, and the Pretendard font license. `waid-desktop.exe --licenses` prints the same complete text when stdout is redirected. The notices are embedded in the executable and require no download or companion files.

## Using the app

The controls below describe Windows. See [Mac controls and current limits](../MACOS.md) for the AppKit interface.

The default window is **600×780 logical pixels**, with expandable session cards in one column and an optional two-column view. Compact cards align the project, remaining context, time, and status badge on the first line, with the latest request and model beneath them. Any last logged status appears on the third line. Expanded cards keep previews scrollable and place the session-open and dismiss actions side by side. Bundled Pretendard SemiBold body text and Bold headings keep text clear. The waid logo appears at the top, and harness logos appear on cards. Drag the title area to move the window and its edges to resize it.
...
- Cards also show the latest logged **remaining context percentage** as **남은 컨텍스트**, between the project and time/date on the same line, when both token count and context limit are available. When **25% or less remains**, the remaining percentage is emphasized; observed compaction also keeps this line emphasized, including when usage is unknown. Both facts appear together when applicable, directly in collapsed cards and the two-column list without opening details. Expand a card for tokens / limit, the measurement's timestamp (UTC), and observed compaction with its timestamp when available. These are measurements, with no quality score, cumulative token total, or account-plan quota. Unknown values are shown as unknown, including a missing context limit; model names are never used to guess one.
...
**PowerShell SSH screen detection (Windows):** while `ssh.exe` is running, waid checks terminal documents exposed by Windows Terminal / classic console UI Automation every five seconds. Recognisable Codex or Claude Code screens appear as **화면 관찰** (screen observation), with an inferred visible request or task label and an explicitly displayed model when available. This also works for an agent visible inside tmux, without remote installation or another SSH login. Screens are bounded to 16 documents × 16,000 UTF-16 units, the helper times out after four seconds, and `WAID_TERMINAL_SCAN=0` disables this path. `waid doctor` reports the observation count and read failures.

Screen observations are heuristic terminal entries, with `terminal_screen` evidence and **Unknown** status/context. They have no provider session ID, confirmed request event, or automatic session-return target. A local terminal screen can also be observed while an unrelated SSH client is running; the screen is not proof of which host owns the agent. Hidden tabs and hidden tmux panes are unavailable, observations disappear when no longer visible, mixed-agent panes are skipped, and a missing/changed agent banner or footer may prevent recognition. The preview can be incomplete or include unsent input. It is not used to restore dismissed entries automatically. Raw screen text is processed in memory; ordinary waid settings/exports can still retain the displayed preview. Full remote logs and hidden tmux collection require separately authenticated access and are not implemented.
...
waid source code is licensed under the [MIT License](../LICENSE). [Third-party notices](../THIRD_PARTY_NOTICES.txt) retain the dependency authors' copyright and license texts. Bundled Pretendard fonts retain their [SIL Open Font License](../desktop/assets/fonts/LICENSE.txt). The OpenAI, Claude and Orca marks used beside sessions retain their owners' rights; [their source record](../desktop/assets/README.md) does not grant a new license or establish SignPath eligibility.
