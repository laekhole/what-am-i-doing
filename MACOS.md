...
- The details pane includes the shared context summary (usage/remaining percentage and observed compaction when available). The current Mac list does not include that summary in its row text, so its context highlight code does not produce a visible list badge. Windows' full token/limit and measurement-timestamp detail block is also not exposed in the Mac pane. Context presentation parity remains v0.3.0 work.
...
The automated AppKit smoke uses isolated `WAID_DATA_DIR` and a Codex transcript fixture. It collects the fixture through the real child process, selects/copies its multiline Unicode prompt, pins/dismisses/restores it, changes search/topmost/opacity, opens the editor and exercises close/reopen. It saves a pin, 75% opacity and Midnight, exits, then verifies all three in a second app process. Daylight and Midnight content-view PNGs are included in the CI artifact for visual review. Rust tests cover persistence, template preview/apply, organization, invalid links, revival and collector-error retention. These are fixture/control checks, not the walkthrough above.
