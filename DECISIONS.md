...
## D40 — 전수 점검의 공통화 범위 (2026-09-23)

기존 코어와 네이티브 셸 경계는 유지한다. CLI·HTML의 수집 필터, JSON·SQLite의 모델 추출, Windows·Mac의 고정/숨김 한도와 빈 화면 안내, CLI·네이티브 메뉴의 라이선스 원문처럼 실제로 어긋난 경로를 공통화한다. `--keys`는 실제 렌더러에서 키를 얻으며 사용자 로그를 수집하지 않는다. 파일 크기만으로 이벤트 처리와 Win32 코드를 옮기거나 새 의존성을 도입하지 않는다.

외부 로그를 읽는 공용 JSON 파서는 128단계 중첩과 유효한 JSON 숫자·문자열만 허용한다. SQLite 폴백 사본은 조회별로 분리하고 소멸 시 정리하며 DB·WAL·SHM 전체에 256 MiB 상한을 적용한다. 이는 쓰기 중인 원본 DB를 원자적으로 복사한다는 보장이 아니다. 로컬 대시보드는 루프백 바인딩에 더해 Host·메서드·헤더 크기와 대기 시간을 제한한다. 구버전 `waid-*.md`는 과거 스냅샷임을 표시하고 현재 문서로 연결한다.

수정 항목, 조사 범위와 플랫폼별 확인 수준은 [전수 점검 기록](VALIDATION.md#전수-점검과-공통-경로-정리--2026-09-23)에 둔다.
...
| 확인한 사실 | 설계에 주는 의미 |
|---|---|
| [README](docs/USER_GUIDE.md#version-roadmap)는 v0.4.0 Android·waidaway LAN을 모바일 브라우저 우선으로 제안한다. 원안은 앱 설치·기기 키 저장을 전제한다. | 클라이언트 형태를 정하기 전 mDNS·Keychain·백그라운드 연결을 공통 기능으로 확정할 수 없다. |
...
## D36 — 창을 숨긴 동안의 트레이와 답변 알림 (2026-09-14)

**선택:** 기존 Win32 트레이·수집기·상태 판정·세션 복귀를 재사용한다. 트레이에서 요약과 내 차례 세션 최대 8개를 보여주며, 새 답변을 먼저 정렬한다. 임시 검색과 상태/에이전트 필터는 트레이 범위에 적용하지 않고, 기본 최근 목록의 숨김·제외·보조 세션 정책은 적용한다. 전체 내 차례 보기와 세션 바로가기는 기존 창의 필터를 초기화하고 Waiting 목록을 연다. 돌아갈 창을 확인할 수 없으면 마지막 답변을 포함한 상세를 표시한다.

**상태와 알림:** 신뢰할 수 있는 로그/훅에서 실행 이후 관측한 요청의 종료만 대상으로 삼는다. 세션 전체 ID와 요청 마커를 함께 사용해 Waiting→Waiting 사이의 새 요청도 구분하며 같은 요청의 재등장·반복 스냅샷은 재알림하지 않는다. 원래 Waiting 상태와 waid의 새 답변 표시는 별개다. 직접 카드/상세 열기 또는 명시적인 표시 지우기로 새 표시를 해제하며 원본 앱에서 읽었는지는 추정하지 않는다. 제외 중 완료된 답변은 다시 보이게 해도 재알림하지 않는다. 기록은 이번 프로세스에서 유지하고 재실행 시 기존 시작 기준을 따른다.

**알림 수단:** 포터블 EXE의 기존 `Shell_NotifyIconW`를 사용하며 새 SDK·서비스·설치 등록은 추가하지 않는다. 2초 동안 답변을 묶고 프로젝트·에이전트만 무음으로 알린다. waid가 실제 전경 창일 때와 OS가 알림 수신에 부적절한 상태라고 응답하면 배너를 생략하되 새 답변 표시는 남긴다. 네이티브 알림 콜백에는 개별 ID가 없으므로 표시 중인 배너의 대상은 후속 답변으로 교체하지 않는다. OS가 배너를 억제할 수 있고 알림 센터 보관은 보장하지 않는다. 지속적인 알림 센터 기록이 제품 요구가 되면 별도 Windows 앱 알림 등록을 검토한다. [알림 데이터·무음·quiet time·realtime](https://learn.microsoft.com/en-us/windows/win32/api/shellapi/ns-shellapi-notifyicondataw), [알림 가능 상태 조회](https://learn.microsoft.com/en-us/windows/win32/api/shellapi/nf-shellapi-shqueryusernotificationstate), [콜백과 인터페이스 버전](https://learn.microsoft.com/en-us/windows/win32/api/shellapi/nf-shellapi-shell_notifyiconw).

**사용자 제어:** 알림 켜기/끄기와 첫 닫기 안내 확인은 기존 설정 파일에 저장한다. 1시간 쉬기는 이번 실행에만 적용하고 생략한 알림은 나중에 몰아서 보내지 않는다. 새 답변과 수집/세션 오류는 로고의 배지와 설명 텍스트로 함께 표시한다. 최소화는 작업 표시줄, 닫기는 트레이, 종료는 수집기를 포함한 waid 종료로 유지한다. 전역 단축키·별도 요약 패널·자동 시작은 이번 구현에 넣지 않는다.

검사 결과와 실환경 한계는 [트레이 검증 기록](VALIDATION.md#트레이-세션-요약과-답변-알림--2026-09-14)에 둔다.

## D37 — 한국어·영어 전환과 공개 소개 준비 (2026-09-14)

제품 문구는 `Language`, `tr`, 컴파일 시 양쪽 형식 문자열을 검사하는 `trf!`로 선택한다. 별도 번역 라이브러리나 번역 서비스는 추가하지 않는다. 상태 식별자·로그 파싱·JSON 스키마와 실제 프로젝트·요청·답변은 유지한다. Windows의 버튼·메뉴·트레이와 Mac의 AppKit 선택기는 기존 설정 파일의 `language`에 `ko` 또는 `en`을 저장하고 표시를 즉시 갱신한다.

새 설정은 한국어 시스템에서 한국어, 그 외에는 영어를 선택한다. Windows는 [GetUserDefaultUILanguage](https://learn.microsoft.com/en-us/windows/win32/api/winnls/nf-winnls-getuserdefaultuilanguage)를 사용한다. `WAID_LANG`은 자동 감지를 재정의하며, 저장된 선택은 환경변수보다 우선한다. 언어 필드가 없는 기존 설정은 한국어로 읽어 기존 사용 경험을 유지한다. CLI의 `--lang`은 해당 실행의 선택이며 데스크톱 설정을 바꾸지 않는다. 로컬 HTML은 요청 URL의 언어를 렌더러에 직접 넘기므로 서로 다른 언어로 접속한 브라우저가 전역 상태를 바꾸지 않는다.

수집기는 시작 시 언어를 전달받는다. 실행 중 언어를 바꿔도 관찰 기준을 초기화하지 않으며, 이미 문자열로 만들어진 수집 진단은 재시작 전까지 시작 언어를 유지한다. 이를 위해 새 IPC나 수집기 재시작 경로를 만들지 않는다. 운영체제가 제공하는 대화상자·오류와 사용자 템플릿은 자동 번역하지 않는다.

공개 범위는 앱 영문화와 [Reddit·GeekNews 소개글 초안](LAUNCH.md)까지다. 웹사이트 배포나 실제 게시를 하지 않으며, 영어 기능이 없는 기존 v0.2.0 배포본과 현재 소스를 구분한다. 검사는 [언어 검증 기록](VALIDATION.md#한국어영어-전환과-소개글-준비--2026-09-14)에 둔다.

## D38 — 오픈소스 Windows 코드 서명 준비 (2026-09-14)

개인 오픈소스 배포에는 SignPath Foundation 무료 지원 신청을 우선 준비한다. 기존 MIT 선언의 전문과 공개 서명 정책·신청 초안을 보완하고, Cargo 버전에서 EXE 제품명·버전 정보를 생성한다. 서명자 명의는 인증서 발급 전에 제품 메타데이터에 넣지 않는다. 신청 초안과 실제 승인 상태는 [Code signing policy](SIGNING.md)에 구분한다.

평판 심사와 외부 계정 승인이 필요한 단계이므로 승인되지 않은 서비스의 워크플로우 설정을 미리 활성화하지 않는다. 승인 후 기존 GitHub 빌드·패키징·Sigstore 흐름에 Authenticode 서명·타임스탬프·검증을 추가하고, 체크섬과 Sigstore 서명은 최종 서명된 EXE를 대상으로 생성한다. 현재 릴리스는 여전히 Authenticode 미서명이며 이번 준비로 Windows 차단 해소를 주장하지 않는다. 확인 결과는 [서명 준비 검증](VALIDATION.md#오픈소스-서명-신청과-exe-메타데이터-준비--2026-09-14)에 기록한다.

## D39 — 단일 EXE 라이선스 고지 (2026-09-14)

프로젝트 MIT·검토한 의존성/표준 라이브러리 고지·폰트 OFL 원문을 컴파일 시 하나의 텍스트로 내장한다. 기존 Windows 트레이 메뉴와 `--licenses`가 동일한 본문을 사용하며, 메뉴는 기존 원자적 파일 저장 함수로 앱 데이터 폴더에 고지를 쓰고 기본 텍스트 앱으로 연다. 별도 다운로드나 동반 파일 없이 포터블 EXE 안에 고지가 남는다.

빌드마다 새 라이선스 도구를 실행하는 대신 실제 라이선스에서 수집한 [고지 스냅샷](THIRD_PARTY_NOTICES.txt)을 검토한다. Cargo 의존성 또는 Rust 도구체인이 바뀌면 이 본문도 갱신해야 한다. 타사 로고의 권리 고지는 이용 허가나 SignPath의 OSS 요건 충족을 대신하지 않으며, 이번 요청에서는 로고를 변경하지 않는다. 검사는 기존 단일 EXE 통합 검사에서 원문 보존과 동반 파일 불필요를 확인한다.
