...
Status (2026-09-14): **preparing an application; not submitted or approved**. No waid release currently has SignPath Foundation Authenticode signing. The published v0.2.0 release has a detached Sigstore signature. See [download verification](README.md#download-and-verify-a-release).
...
waid collects coding-agent logs locally and has no telemetry or automatic upload of session data. User-requested links, integrations, and custom HTML templates have the limits described in the [privacy and local operation notice](README.md#privacy-and-local-operation).
